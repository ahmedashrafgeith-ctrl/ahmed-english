document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadReferralsPanel() {
    ...
    const refCfg = (window.APP_CONFIG && APP_CONFIG.referral) || {};
    const sheetApp = refCfg.sheetUrl || '';
    const sheetBtn = document.getElementById('ref-sheet-btn');
    if (sheetBtn) {
      sheetBtn.style.display = 'inline-flex';
      if (refCfg.spreadsheetUrl) sheetBtn.href = refCfg.spreadsheetUrl;
    }
        ...
        if (fromSheet) {
          try {
            const res = await fetch(sheetApp, {
              ...
              body: JSON.stringify({ action: 'status', row: Number(row), status: act })
            });
            const data = await res.json();
            if (!res.ok || (data && data.ok === false)) throw new Error((data && data.message) || 'sheet http ' + res.status);
          } catch (err) {
            if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#DC2626'; msgEl.textContent = 'Sheet update failed: ' + (err && err.message || err); }
            b.disabled = false;
            return;
          }
        } else {
    ...
    if (sheetApp) {
      try {
        ...
        if (res.ok && data && data.ok && Array.isArray(data.rows)) {
          render(data.rows.slice().reverse());
          return;
        }
    ...
    try {
      ...
      render(rows);
    } catch (e) { console.error('referrals db fallback error:', e); }
  }
